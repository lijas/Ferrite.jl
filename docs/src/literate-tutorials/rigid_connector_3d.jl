...
grid = togrid("docs/src/literate-tutorials/plate_hole_3d.geo")
...
rigidbody_node = Node(Vec((5.0, 5.0, 0.5)))
...
ip_rb_u = Lagrange{Ferrite.RefPoint, 1}()^3
ip_rb_θ = Lagrange{Ferrite.RefPoint, 1}()^3
...
add!(ch, rb)
add!(ch, Dirichlet(:u, getfacetset(grid, "PlateRightLeft"), x -> (0.0, 0.0, 0.0)))
add!(ch, Dirichlet(:u, getvertexset(grid, "rigidvertex"), x -> (0.0, 1.0, 0.0)))
add!(ch, Dirichlet(:euler_angles, getvertexset(grid, "rigidvertex"), x -> (0.0,0.0,0.0)))
close!(ch)
...
C = gradient(ϵ -> 2 * Gmod * dev(ϵ) + 3 * Kmod * vol(ϵ), zero(SymmetricTensor{2, dim}));
...
function calculate_stresses(grid, dh, cv, u, C, cellset)
    dim = Ferrite.getspatialdim(grid)
    qp_stresses = [
        [zero(SymmetricTensor{2, dim}) for _ in 1:getnquadpoints(cv)]
