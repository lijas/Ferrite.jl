...
dim = 3
ip = Lagrange{RefPyramid,2}() #Ferrite.default_interpolation(getcelltype(grid))
refshape = Ferrite.getrefshape(ip)
qr = QuadratureRule{refshape}(6)
...
function assemble_global(cellvalues::CellValues, K::SparseMatrixCSC, dh::DofHandler)
    ...
    for cell in CellIterator(dh)
        ...
        _,_,dV = assemble_element!(Ke, fe, cellvalues, coords, rhs)
        V+=dV
